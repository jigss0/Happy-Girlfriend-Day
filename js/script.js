...
/* =====================================================
   TINY SVG ART GENERATOR (used for gallery + timeline "photos")
===================================================== */
function sceneArt(palette, seed){
  const [c1, c2] = palette;
  const moonX = 40 + (seed % 5) * 12;
  const hillShift = (seed % 4) * 10;
  return `
  <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" style="width:100%;height:100%;display:block;">
    <defs>
      <radialGradient id="g${seed}" cx="50%" cy="30%" r="80%">
        <stop offset="0%" stop-color="${c1}"/>
        <stop offset="100%" stop-color="${c2}"/>
      </radialGradient>
    </defs>
    <rect width="200" height="200" fill="url(#g${seed})"/>
    <circle cx="${moonX}" cy="55" r="22" fill="#fff6e6" opacity="0.85"/>
    <path d="M0 ${140+hillShift} Q60 ${105+hillShift} 120 ${135+hillShift} T200 ${130+hillShift} V200 H0 Z" fill="${c2}" opacity="0.9"/>
    <path d="M20 ${170} c-2-3 2-5 4-2 c2-3 6-1 4 2 c-1.5 2.5-4 4-4 4 s-2.5-1.5-4-4z" fill="#fff" opacity="0.8"/>
    <path d="M170 ${60} c-2-3 2-5 4-2 c2-3 6-1 4 2 c-1.5 2.5-4 4-4 4 s-2.5-1.5-4-4z" fill="#fff" opacity="0.7"/>
  </svg>`;
}
...
function initDiary(getAudioCtx){
  ...
  function renderPage(i){
    ...
    pageLeft.innerHTML = `<div class="diary-quote-wrap" style="height:100%;display:flex;align-items:center;justify-content:center;text-align:center;">
        <p style="font-family:var(--font-display); font-size:1.8rem; color:var(--rose-gold-deep); line-height:1.4;">&ldquo;${p.title}&rdquo;</p>
      </div>`;
    ...
    if(item.type === "filmstrip"){
      el.innerHTML = `
        <div style="position:relative;">
          ...
          <div class="filmstrip-frame">${sceneArt(item.palette, idx)}</div>
        </div>
        <p class="gallery-caption" style="color:#f3e3da;">${item.caption}</p>
      `;
    } else {
      el.innerHTML = `
        <div class="gallery-photo-art">${sceneArt(item.palette, idx)}</div>
...
function initTimeline(){
  ...
  TIMELINE_ITEMS.forEach((item, idx)=>{
    ...
    el.innerHTML = `
      ...
      <div class="timeline-card" tabindex="0" role="button" aria-expanded="false">
        <div class="timeline-date">${item.date}</div>
        <div class="timeline-title">${item.title}</div>
        <div class="timeline-caption">
          <div class="timeline-photo">${sceneArt(item.palette, idx+50)}</div>
  ...
  REASONS.forEach((reason, idx)=>{
    ...
    card.innerHTML = `
      <div class="flip-card-inner">
        <div class="flip-card-front"><span class="num">#${idx+1}</span><span>tap to reveal</span></div>
...
function initScratchCards(){
  ...
  SCRATCH_CARDS.forEach((card, idx)=>{
    ...
    wrap.innerHTML = `
      <div class="scratch-postcard-bg">${sceneArt(card.palette, idx+100)}</div>
